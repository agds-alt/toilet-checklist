// ============================================
// components/ChecklistTable.tsx - COMPACT VERSION (2 tables fit on screen)
// ============================================
'use client';
                            ...
                            <table className="w-full">
                                <thead>
                                    <tr className="bg-gradient-to-r from-slate-100 via-blue-50 to-slate-100">
                                        <th className="px-3 py-2 text-left text-xs font-bold text-slate-800 min-w-[140px] sticky left-0 bg-slate-100/95 backdrop-blur-sm z-10">
                                        ...
                                        {period.days.map(day => (
                                            <th key={day} className="px-2 py-2 text-center min-w-[50px]">
                                                <div className="font-bold text-slate-800 text-xs">{day}</div>
                                ...
                                <tbody>
                                    {locations.map((location, locIdx) => (
                                        <tr key={locIdx} className="border-b border-slate-100 hover:bg-blue-50/30 transition-colors group">
                                            <td className="px-3 py-2 text-xs font-semibold text-slate-800 sticky left-0 bg-white/95 backdrop-blur-sm group-hover:bg-blue-50/95 transition-colors z-10">
                                                <div className="flex items-center gap-1.5">
                                            ...
                                            {period.days.map(day => {
                                                ...
                                                return (
                                                    <td
                                                        ...
                                                        className={`px-1 py-2 text-center score-cell ${getCellColor(score)} cursor-pointer`}
                                                    >
                                                        <div className="relative flex flex-col items-center justify-center min-h-[40px]">
                                                            {score ? (
                                                                <>
                                                                    <span className="text-base font-bold">{score}</span>
                                                                    {hasPhotoFlag && (
                                                                        <div className="absolute top-0 right-0 bg-blue-600 p-0.5 rounded-md shadow-sm">
            ...
            <div className="glass-card rounded-xl p-4 shadow-lg">
                ...
                <div className="grid grid-cols-5 gap-2">
                    {[
                        ...
                        <div key={idx} className={`${item.color} border-2 rounded-lg p-2 text-center hover:scale-105 transition-transform`}>
                            <div className="text-lg mb-0.5">{item.icon}</div>
                            <div className="font-bold text-slate-800 text-[10px]">{item.label}</div>
